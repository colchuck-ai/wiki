...
- **`index_status(scope?) → stale_paths[]`** — the read-only index-freshness check: returns the paths in `scope` (default: whole bundle) whose on-disk `index.md` differs from what `reindex` would regenerate from the concepts currently on disk, and **writes nothing**. It is `reindex`'s derivation without the write — the same computation, compared against the materialized file instead of replacing it — so C005 stays the one owner of index derivation while a read-only caller (C011's Lint) can detect out-of-band index drift without mutating. An empty result means every `index.md` in scope is current. This is the concrete freshness signal C011 reads for Lint, in place of ever calling the mutating `reindex` (see [ADR016](../drs/ADR016-survey-lint-aggregation-ownership.md), [CR010](../../crs/CR010-read-only-freshness-interface.md)).
...
- **Corpus changed outside the verb surface** (a file added, moved, or removed by a raw edit on disk, not through a write verb): the on-disk `index.md` is stale for that path until the next `reindex`. The write verbs themselves cannot leave the index stale — each fires `reindex` as an internal effect ([ADR011](../drs/ADR011-concept-verb-surface.md)), so there is no in-band "forgot to reindex" drift to worry about — but C005 does not watch the corpus, so an out-of-band mutation goes undetected until a read-only Lint/Survey freshness check surfaces it — `index_status`, which reports the stale paths and writes nothing (never `reindex`, which would mutate). Not a runtime guarantee C005 enforces. A change that alters neither a concept's index entry (title/description) nor its presence — an edit to body prose alone — leaves `index.md` correct and so is invisible to `index_status`; that residual is the irreducible cost of recording by intent rather than watching the corpus ([ADR011](../drs/ADR011-concept-verb-surface.md)).
...
- **C011 - Curation Operations**: reads `dangling_links` as the referential-integrity face of its **Lint** aggregation, and reads `index_status` as the read-only index-freshness signal for Lint's freshness check — never `reindex`, so a Lint pass mutates nothing (see [ADR016](../drs/ADR016-survey-lint-aggregation-ownership.md), [CR010](../../crs/CR010-read-only-freshness-interface.md)). The recency half of that freshness check is C007's `recency_status`, not C005's. No reverse dependency; C005 is unaware it is aggregated.
...
- **No hidden mutation**: `inbound_links`, `dangling_links`, `graph`, and `index_status` never write to the corpus; `reindex` writes only `index.md` files, never concept content or links.
- **Read-only freshness fidelity**: `index_status(scope)` returns exactly the paths whose on-disk `index.md` differs from a fresh regeneration, and writes nothing — verifiable by editing a concept's title or description on disk outside the verb surface, confirming its directory appears in `index_status`, then confirming the corpus is byte-identical before and after the call.
...
- The read-only **`index_status`** dry-run exists because Lint mutates nothing yet must detect the one residual drift [ADR011](../drs/ADR011-concept-verb-surface.md) leaves — a corpus changed outside the verb surface. The interface was deferred by [ADR016](../drs/ADR016-survey-lint-aggregation-ownership.md) as a separately-tracked concern and added in [CR010](../../crs/CR010-read-only-freshness-interface.md). The alternatives — having Lint re-derive the index itself (duplicates C005's derivation, breaking one-owner) or call `reindex` and treat an empty `updated_paths` as "fresh" (a mutation on a read path) — were both rejected for the dry-run, which is `reindex`'s derivation minus the write.
