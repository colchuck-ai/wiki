...
- **`triage`** — the disposition block, appended to the task record:
  ...
  - **`direction`** — present only on an admitted item, the reconciliation steer handed to C003: `{ mode ∈ { merge, keep-new }, target-concept-id? , note? }`. `target-concept-id` is required when `mode = merge` and must resolve to an existing concept (checked by C003 at authoring time via C004), but it need not be one of `overlap.candidates` — the steward may name any concept they already know is the right target, whether or not C002's scan surfaced it. `note` carries any distillation hint. (`discard` is expressed as a `rejected` status, not a direction.)
...
- **`record_disposition(task_id, decision, [direction]) → task`** — writes the steward's confirmed disposition and advances `status`. On `admit`, records `direction` (with a `target-concept-id` when merging) for C003 — `target-concept-id` may be any existing concept the steward names, not only one of `assess_overlap`'s candidates. On `reject`, calls `C001.discard(task)`. On `hold`, sets `status = held`. This is the only call that changes a task's status.
...
- **Steward already knows the merge target and it's not among the surfaced candidates** (e.g. new material they know updates a specific concept the semantic scan missed, or scored low): recorded as an ordinary `direction = { mode: merge, target-concept-id }` all the same — C002 does not require the target to have been recommended, only that it resolves to an existing concept.
...
- **Concrete merge target**: whenever the steward chooses merge, the recorded `direction` names a specific existing `concept-id` C003 can act on — whether or not that id came from `overlap.candidates`.
