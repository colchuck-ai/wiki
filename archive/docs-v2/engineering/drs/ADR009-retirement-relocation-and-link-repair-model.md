...
> **Superseded in part by [ADR012](ADR012-repair-to-successor-link-model.md) and [ADR011](ADR011-concept-verb-surface.md):** the per-operation repair split (retarget on `relocate`, strip on `retire`) is unified into the single repair-to-successor rule, and the `retire`/`relocate` operations are renamed `delete`/`move` — with `delete` gaining an optional replacement whose inbound links are redirected rather than stripped. The free choice of mechanism, mechanical-repair-only, no-dangling-end-state, review-candidates, and self-contained candidate sweep recorded below are retained.
