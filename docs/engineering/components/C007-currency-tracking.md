...
C007 is a **write-triggered recorder**, not a judge: it records the event a caller declares — it never diffs a concept's before/after content to decide for itself whether a change was meaningful. This is sound only because every corpus-mutating path is itself a deliberate, steward-directed operation (C003's `author`, C010's `declare_charter`/`revise_charter`, and eventually C008's lifecycle actions) — the architecture's "intent in, work out" principle already rules out the kind of silent or cosmetic hand-edit that would need a meaningfulness filter. C007 fulfills recorded recency (O002-R001) and change history (O002-R002) outright. Unlike C005's graph or C006's and C010's findings — all recomputed on demand and never persisted — C007's two artifacts are durable corpus writes, so a consumer with no Wiki tooling can read a concept's recency and history straight off the files ([ADR008](../drs/ADR008-recency-materialization-and-log-scoping.md)).
...
- **Change-log entry** — an OKF `log.md` entry (§7): date-grouped, newest first, `**<Kind>**: <prose>` naming the concept with a bundle-relative link, rendered and parsed through C004's log-entry helper. Wiki fixes a **closed kind vocabulary** — `Creation`, `Update`, `Deprecation` — a Wiki convention layered on OKF §7's leading-bold-word convention (itself non-normative); a merge is recorded as `Update` with the merge described in prose, not a fourth kind.
...
- **`record(concept_id, kind, summary) → { timestamp, log_entry }`** — the single write entry point, mirroring C005's `reindex` as the sole mutation a writer triggers. Sets the concept's `timestamp` (via C004) to the moment of the call and appends one dated entry — `kind ∈ { Creation, Update, Deprecation }`, `summary` the prose — to the `log.md` in the concept's own directory (via C004). Called as the terminal step of any operation that mutates a concept's content or status: C003 after `author` (`Creation` for `keep-new`, `Update` for `merge`), C010 after `declare_charter` (`Creation`) or `revise_charter` (`Update`), and eventually C008 after a deprecation/retirement (`Deprecation`) or relocation (`Update`).
...
- **Concept relocated to a different directory** (a future C008 operation): prior entries stay in the old directory's `log.md`, since entries are never rewritten or moved; `history(concept_id)`, scoped to the concept's current directory, will not surface them after relocation. A known imprecision, acceptable at the low-volume, single-steward scale — `history_all` at the bundle root still surfaces every entry regardless of which directory holds it.
...
- **Charter revised in place (C010)**: `record` is called against the one charter concept's existing id and directory — no relocation, no new file, consistent with C010's revise-in-place contract.
...
- **C008 - Lifecycle & Retirement** (future): will call `record` after a deprecation/retirement (`Deprecation`) or relocation (`Update`), and will read `recency` as one input to its staleness signal alongside C006's drift findings (O003-R002) — to be specced against this contract.
...
- The closed change-kind vocabulary (`Creation` / `Update` / `Deprecation`) is a Wiki convention layered on OKF §7's non-normative leading-bold-word convention — recorded here rather than in a separate ADR, the same treatment C004 gave the kebab-case slug convention; promote it if it becomes contested.
