...
- **Promote at authoring, cite by path.** The durable source C001 held in staging is promoted into `/references/` as a git-tracked asset and cited by bundle-relative path. This is the physical move C001's promotion boundary deferred here. An in-place source already version-controlled alongside the corpus is cited at its existing path with no copy. Because nothing outside the task pointed at the staging path, the one-time relocation is safe (ADR002).
...
- **Write the concept, not the index, log, or recency.** C003 writes the concept and its citations and links. The navigable index (C005) and the recency and change history (C007) are maintained by their owners over the corpus C003 produces. Authoring is the first event in a concept's history for C007 to record; whether C003 emits the initial `timestamp` through C004 or leaves the value to C007 is settled when C007 is specced (C004 assigns the `timestamp` value to C007).
...
- **Source drifts after authoring**: not C003's concern — C006 - Source Revalidation detects drift against the content-hash baseline C001 captured for the promoted asset.
...
- **C007 - Currency Tracking**: authoring is the first event in a concept's history; recording recency (the `timestamp` value) and the dated change log is C007's, consistent with C004 assigning the `timestamp` value to C007. C003 emits the concept through C004; whether it also populates the initial `timestamp` or leaves that to C007 is settled when C007 is specced.
- **C006 - Source Revalidation**: relies on the promoted provenance asset and the content-hash baseline as the form a claim was drawn from, revalidated only after the item is a concept.
