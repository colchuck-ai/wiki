...
- **`reindex(scope?) → updated_paths[]`** — regenerates the `index.md` for `scope` (a directory) and, when omitted, the whole bundle, from the concepts currently on disk (read through C004). Idempotent: re-running with no intervening corpus change returns an empty `updated_paths[]` and writes nothing. Called by any component that writes, deprecates, retires, or relocates a concept — C003 after authoring, C008 last in every one of its operations — since C005 does not watch the corpus.
- **`inbound_links(concept_id) → link[]`** — every concept in the corpus that links to `concept_id`, computed by scanning the corpus fresh. The concrete mechanism for O005-R003: called by C008 before completing a `retire` or `relocate`, so it knows exactly which referring concepts to mechanically repair — unlink to plain text for a retirement, retarget to the new path for a relocation — before either operation is considered done. Deprecation needs no such repair, since the concept's file simply stays where it is.
...
- **Link to a concept later retired or relocated by C008**: never observed as dangling in steady state — C008 reads `inbound_links` and mechanically repairs every one (unlink or retarget) as part of the same operation, before considering it complete.
...
- **C008 - Lifecycle & Retirement**: the one component with a real dependency on C005 (see [ADR006](../drs/ADR006-index-materialization-and-graph-computation.md)). C008 calls `inbound_links` before completing a `retire` or `relocate`, to know exactly which referring concepts to mechanically repair, and calls `reindex` last in every operation — after a deprecation (marker changed), a retirement (concept removed), or a relocation (path and referring links changed).
